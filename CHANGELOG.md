...
- Feature: **Agent 基准校准（`npm run bench:agents`）**。对每个可用执行器 × 任务套件（内置 3 个确定性任务或 `--tasks` 自定义）在全新临时 git 仓库跑真实 job（种子先提交、任务间互不污染），汇总成功率/均值 token/墙钟/分类分布——与战绩层同口径，跑在真实 workspace 时结果自然喂给路由。任一执行器 0 成功非零退出；会真实调用编码 CLI，仅适合显式触发。
