...
- `src/ui.ts` owns the read-only HTTP server, JSON/artifact routes, SSE stream, and the TUI entry point.
...
4. Preserve the loopback-only server binding and the distinction between Bearer-authenticated APIs and query-token SSE.
5. Add a focused `node:test` regression for changed behavior, then run the repository checks listed in [Quality Guidelines](./quality-guidelines.md).
