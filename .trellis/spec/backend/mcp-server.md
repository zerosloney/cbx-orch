...
- Tools: `cbx_start` `cbx_status` `cbx_review` `cbx_continue` `cbx_artifact` `cbx_cancel` `cbx_approve` `cbx_list` `cbx_logs` `cbx_result` `cbx_queue` `cbx_queue_pause` `cbx_queue_resume` `cbx_retry` `cbx_review_gate`.
