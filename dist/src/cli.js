...
async function main() {
    ...
    if (["run", "start"].includes(command)) {
        ...
        const defaults = mergeConfig(fileConfig, {
            ...
            trustMode: option(args, "--trust-mode"),
        });
        const existingJob = option(args, "--job-id");
        let jobId = existingJob;
        if (!jobId) {
            if (!task)
                throw new Error("请提供 --task 或 --task-file。");
        ...
        print(result);
        if (has(args, "--ci") && result.status !== "done")
            process.exitCode = 2;
        return;
    }
    if (command === "mcp") {
    ...
    if (command === "list") {
        print(await listJobs(workspace));
        return;
    }
    if (command === "queue") {
        const action = args.find(value => !value.startsWith("--") && value !== workspace);
        if (action === "pause")
            print(await pauseQueue(workspace));
        else if (action === "resume")
            print(await resumeQueue(workspace));
        else
            print(await listQueue(workspace));
        return;
    }
    if (command === "dispatch") {
    ...
    if (command === "logs") {
        console.log(await readArtifact(workspace, args[0], "events.ndjson"));
        return;
    }
    if (command === "files") {
        try {
            print(JSON.parse(await readArtifact(workspace, args[0], "result.json")));
        }
        catch {
    ...
    if (command === "result") {
        console.log(await readArtifact(workspace, args[0], "result.json"));
        return;
    }
    if (command === "watch") {
        ...
        while (true) {
            ...
            if (["done", "failed", "needs_fix", "review_failed", "cancelled"].includes(state.status)) {
                if (has(args, "--ci") && state.status !== "done")
                    process.exitCode = 2;
                return;
            }
    ...
    if (command === "tui") {
        await runTui(workspace, Number(option(args, "--interval-ms", "1000")));
        return;
    }
    if (command === "review") {
        try {
            console.log(await readFile(`${jobDir(workspace, args[0])}/review.md`, "utf8"));
        }
        catch {
    ...
    if (command === "cancel") {
        print(await cancelJob(workspace, args[0]));
        return;
    }
    if (command === "approve") {
    ...
    if (command === "retry") {
        print(await retryQueueJob(workspace, args[0], Number(option(args, "--priority", "0"))));
        return;
    }
    if (command === "clean") {
