...
import { loadExecutorPlugin } from "./executor.js";
async function main() {
    const [executor, workspace, requestFile] = process.argv.slice(2);
    if (!executor || !workspace || !requestFile)
        throw new Error("plugin host 缺少参数");
    const request = JSON.parse(await readFile(requestFile, "utf8"));
    const plugin = await loadExecutorPlugin(executor, workspace, request.plugin?.policy, request.plugin?.sha256);
