...
import { health, listQueue, loadState, readArtifact } from "./core.js";
...
function json(res, value, status = 200) { res.writeHead(status, { "content-type": "application/json; charset=utf-8", "x-content-type-options": "nosniff", "cache-control": "no-store" }); res.end(JSON.stringify(value)); }
function text(res, value, contentType = "text/plain; charset=utf-8") { res.writeHead(200, { "content-type": contentType }); res.end(value); }
export function createWebUiServer(workspace, host = "127.0.0.1", port = 4173) {
    if (!new Set(["127.0.0.1", "localhost", "::1"]).has(host))
        throw new Error("Web UI 仅允许绑定到本机回环地址；远程访问需要在受认证的反向代理后显式实现。");
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (url.pathname === "/")
                return text(res, page, "text/html; charset=utf-8");
            if (url.pathname === "/events") {
                res.writeHead(200, { "content-type": "text/event-stream", "cache-control": "no-cache", connection: "keep-alive" });
                clients.add(res);
                res.write(`data: ${JSON.stringify({ at: new Date().toISOString(), type: "connected" })}\n\n`);
                req.on("close", () => clients.delete(res));
                return;
            }
            if (url.pathname === "/api/jobs")
                return json(res, await import("./core.js").then(mod => mod.listJobs(workspace)));
            if (url.pathname === "/api/queue")
                return json(res, await listQueue(workspace));
            if (url.pathname === "/healthz" || url.pathname === "/api/metrics")
                return json(res, await health(workspace));
            const job = /^\/api\/jobs\/([^/]+)$/.exec(url.pathname);
            if (job)
                return json(res, await loadState(workspace, job[1]));
            const artifact = /^\/api\/jobs\/([^/]+)\/artifact\/([^/]+)$/.exec(url.pathname);
            if (artifact)
                return text(res, await readArtifact(workspace, artifact[1], artifact[2]));
    ...
    const heartbeat = setInterval(() => { const message = `data: ${JSON.stringify({ at: new Date().toISOString(), type: "heartbeat" })}\n\n`; for (const client of clients)
        client.write(message); }, 1500);
    heartbeat.unref();
    server.on("close", () => clearInterval(heartbeat));
    return server;
}
export async function startWebUi(workspace, port = 4173, host = "127.0.0.1") {
    const server = createWebUiServer(workspace, host, port);
    await new Promise(resolve => server.listen(port, host, resolve));
    console.log(`CBX UI: http://${host}:${port}`);
    await new Promise(resolve => server.on("close", resolve));
}
export async function runTui(workspace, intervalMs = 1000) {
    let stopped = false;
    ...
    while (!stopped) {
        ...
        for (const job of jobs.slice(0, 30))
            process.stdout.write(`${job.jobId.padEnd(28)} ${String(job.status).padEnd(16)} ${job.phase}\n`);
