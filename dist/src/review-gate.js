 ...
 */
export async function runReviewGate(workspaceInput, options = {}) {
    ...
    if (result.timedOut)
        return { pass: true, reason: `审查超时（${timeoutMs}ms），fail-open 放行；可手动跑 cbx review-gate 复核`, review: "", verdict: "TIMEOUT" };
    if (result.code !== 0)
        return { pass: true, reason: `审查代理退出码 ${result.code}，fail-open 放行；可手动跑 cbx review-gate 复核`, review: "", verdict: "ERROR" };
    let review = "";
    try {
    ...
    if (pass)
        return { pass: true, reason: "审查通过", review, verdict: "PASS" };
    if (fail)
        return { pass: false, reason: `审查发现问题：\n${review}`, review, verdict: "FAIL" };
    return { pass: true, reason: `审查输出无法解析 VERDICT（fail-open 放行）；首行：${firstLine || "<空>"}`, review, verdict: "UNKNOWN" };
}
export function gateEnabled(enabled) { return enabled === true; }
export function stopGateDecision(result) {
    if (result.pass)
        return null;
    return { decision: "block", reason: result.reason };
}
export async function shouldRunGate(workspace) {
    const config = await loadConfig(workspace);
    return config.reviewGate?.enabled === true;
}
/** 供 hooks/stop-review-gate.js 调用的入口：返回 JSON decision 或 null（放行）。fail-open。 */
export async function stopReviewGateHook(workspaceInput) {
    const workspace = path.resolve(workspaceInput);
    if (!(await shouldRunGate(workspace)))
        return null;
    try {
