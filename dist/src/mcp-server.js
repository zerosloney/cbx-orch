...
function text(value) { return { content: [{ type: "text", text: JSON.stringify(value, null, 2) }], structuredContent: value }; }
...
async function callTool(name, args) {
    ...
    if (name === "cbx_queue")
        return listQueue(root);
    if (name === "cbx_queue_pause")
        return pauseQueue(root);
    if (name === "cbx_queue_resume")
        return resumeQueue(root);
    if (name === "cbx_retry")
    ...
    if (name === "cbx_review") {
        try {
            return { job_id: id, review: await readFile(`${jobDir(root, id)}/review.md`, "utf8") };
        }
        catch {
            return { job_id: id, review: "尚无 review.md" };
        }
    }
    ...
    if (name === "cbx_cancel")
        return cancelJob(root, id);
    if (name === "cbx_approve") {
        const state = await approveJob(root, id);
        await startBackground(root, id);
        return state;
    }
    if (name === "cbx_logs")
        return { job_id: id, logs: await readArtifact(root, id, "events.ndjson") };
    if (name === "cbx_result")
    ...
    input.on("line", async (line) => {
        ...
        try {
            ...
            if (request.method === "initialize")
                send(request.id, { protocolVersion: "2024-11-05", capabilities: { tools: {}, resources: { subscribe: false, listChanged: false } }, serverInfo });
                ...
                send(request.id, text(await callTool(String(request.params?.name), (request.params?.arguments ?? {}))));
            else
                throw new Error(`未知方法：${request.method ?? "<missing>"}`);
        }
        catch (error) {
            send(requestId, undefined, { code: -32000, message: error instanceof Error ? error.message : String(error) });
        }
    });
}
