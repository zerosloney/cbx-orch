...
function known(value, name, keys) {
    for (const key of Object.keys(value))
        if (!keys.includes(key))
            throw new Error(`${name} 不支持字段：${key}`);
}
function optionalBoolean(value, name) { if (value !== undefined && typeof value !== "boolean")
    throw new Error(`${name} 必须是布尔值。`); }
function optionalString(value, name) { if (value !== undefined && (typeof value !== "string" || !value.trim()))
    throw new Error(`${name} 必须是非空字符串。`); }
function optionalInteger(value, name, minimum, maximum = Number.MAX_SAFE_INTEGER) { if (value !== undefined && (!Number.isInteger(value) || Number(value) < minimum || Number(value) > maximum))
...
export async function loadRuntimeConfig(workspaceInput) {
    ...
    if (config.approval !== undefined) {
        ...
        known(value, "approval", ["beforeRun"]);
    ...
    if (config.execution !== undefined) {
        ...
        if (value.trustMode !== undefined && value.trustMode !== "trusted" && value.trustMode !== "untrusted")
    ...
    if (config.plugins !== undefined) {
        ...
        for (const key of ["allowPaths", "allowSha256"])
        ...
        const hashes = value.allowSha256;
        if (hashes !== undefined && hashes.some(hash => !/^[a-fA-F0-9]{64}$/.test(hash)))
            throw new Error("plugins.allowSha256 必须是 SHA-256 十六进制摘要。");
    }
    for (const [name, fields] of [["notifications", ["webhook", "timeoutMs", "maxRetries", "retryBaseMs"]], ["telemetry", ["enabled", "endpoint", "serviceName", "timeoutMs", "maxRetries", "retryBaseMs"]]]) {
        ...
        if (name === "telemetry" && value.enabled === true && value.endpoint === undefined)
            throw new Error("telemetry.enabled=true 时必须提供 telemetry.endpoint。");
        optionalInteger(value.timeoutMs, `${name}.timeoutMs`, 50);
        optionalInteger(value.maxRetries, `${name}.maxRetries`, 0, 10);
        if (value.retryBaseMs !== undefined && (typeof value.retryBaseMs !== "number" || value.retryBaseMs < 0))
            throw new Error(`${name}.retryBaseMs 必须是非负数。`);
    }
    if (config.governance !== undefined) {
        const value = object(config.governance, "governance");
        known(value, "governance", ["retentionDays", "redactFields", "redactPatterns"]);
        optionalInteger(value.retentionDays, "governance.retentionDays", 1, 3650);
        if (value.redactFields !== undefined && (!Array.isArray(value.redactFields) || value.redactFields.length > 100 || value.redactFields.some(field => typeof field !== "string" || !field.trim())))
            throw new Error("governance.redactFields 必须是最多 100 个非空字符串。");
        if (value.redactPatterns !== undefined) {
            if (!Array.isArray(value.redactPatterns) || value.redactPatterns.length > 100)
                throw new Error("governance.redactPatterns 必须是最多 100 个正则字符串。");
...
export function redactSensitive(value, fields = []) {
    ...
    const visit = (item) => {
        ...
        if (!item || typeof item !== "object")
            return item;
        return Object.fromEntries(Object.entries(item).map(([key, child]) => [key, sensitive.has(key.toLowerCase()) ? "[REDACTED]" : visit(child)]));
    };
    return visit(value);
}
...
export function redactText(text, fields = [], patterns = []) {
    const sensitive = new Set(fields.map(field => field.toLowerCase()));
    let out = text;
    if (sensitive.size > 0) {
        out = text.split("\n").map(line => {
...
function databaseFile(workspace) { return path.join(workspace, ".cbx", "state.sqlite"); }
function migrate(db) {
    ...
    if (version < 2)
        db.transaction(() => { db.exec("CREATE TABLE metadata (key TEXT PRIMARY KEY, value TEXT NOT NULL)"); db.prepare("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(2, now()); })();
    if (version > SCHEMA_VERSION)
        throw new Error("state.sqlite 的 schema 版本高于当前 cbx，拒绝降级运行。");
}
async function database(workspaceInput) {
    const workspace = path.resolve(workspaceInput);
...
async function importLegacyData(workspace, db) {
    if (db.prepare("SELECT value FROM metadata WHERE key = ?").get("legacy_import_v1"))
        return;
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (typeof state.jobId === "string")
                insert.run(state.jobId, JSON.stringify(state), String(state.updatedAt ?? now()));
        }
        catch (error) {
            if (!isMissing(error))
                console.error(`cbx: 跳过无法导入的旧任务 ${entry.name}：${error instanceof Error ? error.message : error}`);
        }
    }
    try {
        ...
        db.transaction(() => { for (const line of lines) {
            ...
            insertFailure.run(record.at ?? now(), JSON.stringify(record));
        } })();
    }
    catch (error) {
        if (!isMissing(error))
...
async function legacyQueue(workspace, db, fallback) {
    const existing = db.prepare("SELECT state_json FROM queue_state WHERE singleton = 1").get();
    if (existing)
...
export async function loadPersistedState(workspace, jobId) { const db = await database(workspace); const row = db.prepare("SELECT state_json FROM jobs WHERE job_id = ?").get(jobId); return row ? JSON.parse(row.state_json) : undefined; }
export async function savePersistedState(workspace, jobId, value) { const db = await database(workspace); db.prepare("INSERT INTO jobs(job_id, state_json, updated_at) VALUES (?, ?, ?) ON CONFLICT(job_id) DO UPDATE SET state_json = excluded.state_json, updated_at = excluded.updated_at").run(jobId, JSON.stringify(value), now()); }
export async function listPersistedStates(workspace) { const db = await database(workspace); return db.prepare("SELECT state_json FROM jobs ORDER BY updated_at DESC").all().map(row => JSON.parse(row.state_json)); }
export async function loadPersistedQueue(workspace, fallback) { return await legacyQueue(path.resolve(workspace), await database(workspace), fallback); }
export async function savePersistedQueue(workspace, value) { const db = await database(workspace); db.prepare("INSERT INTO queue_state(singleton, state_json, updated_at) VALUES (1, ?, ?) ON CONFLICT(singleton) DO UPDATE SET state_json = excluded.state_json, updated_at = excluded.updated_at").run(JSON.stringify(value), now()); }
export async function savePersistedStateAndQueue(workspace, jobId, state, queue) {
...
export async function savePersistedStateAndFinishQueue(workspace, jobId, state, queueId) {
    const db = await database(workspace);
    await legacyQueue(path.resolve(workspace), db, { entries: [] });
    db.transaction(() => {
        const row = db.prepare("SELECT state_json FROM queue_state WHERE singleton = 1").get();
        const queue = JSON.parse(row.state_json);
        const entry = queue.entries?.find(item => item.queueId === queueId);
        if (entry) {
            const status = String(state.status);
            entry.status = status === "done" ? "done" : status === "cancelled" ? "cancelled" : status === "awaiting_approval" ? "awaiting_approval" : "failed";
...
export async function recordDeliveryFailure(workspace, value) { const db = await database(workspace); db.prepare("INSERT INTO delivery_failures(created_at, record_json) VALUES (?, ?)").run(now(), JSON.stringify(value)); }
async function pruneDeliveryFailureArtifact(workspace, cutoff) {
    ...
    let removed = 0;
    const retained = lines.filter(line => { try {
        const record = JSON.parse(line);
    ...
    catch { /* preserve malformed records for manual recovery */ } return true; });
    if (removed)
        await atomicWriteFile(file, retained.length ? retained.join("\n") + "\n" : "");
    return removed;
}
export async function prunePersistedData(workspace, retentionDays) { if (!retentionDays)
    return 0; const cutoff = Date.now() - retentionDays * 86_400_000; const db = await database(workspace); const sqlite = db.prepare("DELETE FROM delivery_failures WHERE created_at < ?").run(new Date(cutoff).toISOString()).changes; return sqlite + await pruneDeliveryFailureArtifact(workspace, cutoff); }
export async function persistedMetrics(workspace) { const db = await database(workspace); const rows = db.prepare("SELECT state_json FROM jobs").all(); const jobsByStatus = {}; let retryingJobs = 0; for (const row of rows) {
    const state = JSON.parse(row.state_json);
    const status = state.status ?? "unknown";
    jobsByStatus[status] = (jobsByStatus[status] ?? 0) + 1;
    if (state.phase === "retrying")
        retryingJobs += 1;
} const queue = await legacyQueue(path.resolve(workspace), db, { entries: [] }); return { jobsByStatus, queueDepth: (queue.entries ?? []).filter(entry => ["queued", "running", "awaiting_approval"].includes(String(entry.status))).length, failedJobs: jobsByStatus.failed ?? 0, retryingJobs, deliveryFailures: Number(db.prepare("SELECT COUNT(*) AS count FROM delivery_failures").get().count) }; }
export async function acquireServiceLease(workspace, name, ttlMs = 45_000) { const db = await database(workspace); const expiresAt = Date.now() + ttlMs; db.transaction(() => { const lease = db.prepare("SELECT owner_pid, expires_at FROM service_leases WHERE name = ?").get(name); if (lease && lease.owner_pid !== process.pid && processAlive(lease.owner_pid))
    throw new Error("已有活跃 serve 实例；每个工作区只允许一个常驻调度器。"); db.prepare("INSERT INTO service_leases(name, owner_pid, expires_at) VALUES (?, ?, ?) ON CONFLICT(name) DO UPDATE SET owner_pid = excluded.owner_pid, expires_at = excluded.expires_at").run(name, process.pid, expiresAt); })(); return async () => { db.prepare("DELETE FROM service_leases WHERE name = ? AND owner_pid = ?").run(name, process.pid); }; }
...
async function replaceFile(source, target) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        try {
            ...
            if (!new Set(["EACCES", "EPERM", "EBUSY"]).has(String(code)) || attempt === 4)
                throw error;
...
export async function loadJson(file, fallback) {
    try {
        ...
        if (fallback !== undefined && isMissing(error))
            return fallback;
        throw error;
    }
}
...
async function staleLock(file, staleAfterMs) {
    let record = {};
    let modifiedAt = 0;
    try {
...
async function reclaimLock(file) {
...
export async function withFileLock(file, action, options = {}) {
    ...
    for (let attempt = 0; !handle; attempt += 1) {
        try {
            ...
            if (await staleLock(file, staleAfterMs) && await reclaimLock(file))
                continue;
            if (attempt >= retries)
                throw new Error(options.busyMessage ?? "锁正在被另一个进程持有，请稍后重试。");
