...
export async function persistedMetrics(workspace: string): Promise<{ jobsByStatus: Record<string, number>; queueDepth: number; failedJobs: number; retryingJobs: number; deliveryFailures: number }> { const db = await database(workspace); const rows = db.prepare("SELECT state_json FROM jobs").all() as Array<{ state_json: string }>; const jobsByStatus: Record<string, number> = {}; let retryingJobs = 0; for (const row of rows) { const state = JSON.parse(row.state_json) as { status?: string; phase?: string }; const status = state.status ?? "unknown"; jobsByStatus[status] = (jobsByStatus[status] ?? 0) + 1; if (state.phase === "retrying") retryingJobs += 1; } const queue = await legacyQueue(path.resolve(workspace), db, { entries: [] }) as { entries?: Array<{ status?: string }> }; return { jobsByStatus, queueDepth: (queue.entries ?? []).filter(entry => ["queued", "running", "awaiting_approval"].includes(String(entry.status))).length, failedJobs: jobsByStatus.failed ?? 0, retryingJobs, deliveryFailures: Number((db.prepare("SELECT COUNT(*) AS count FROM delivery_failures").get() as { count: number }).count) }; }
export async function acquireServiceLease(workspace: string, name: string, ttlMs = 45_000): Promise<() => Promise<void>> { const db = await database(workspace); const expiresAt = Date.now() + ttlMs; db.transaction(() => { const lease = db.prepare("SELECT owner_pid, expires_at FROM service_leases WHERE name = ?").get(name) as { owner_pid: number; expires_at: number } | undefined; if (lease && lease.owner_pid !== process.pid && processAlive(lease.owner_pid)) throw new Error("已有活跃 serve 实例；每个工作区只允许一个常驻调度器。"); db.prepare("INSERT INTO service_leases(name, owner_pid, expires_at) VALUES (?, ?, ?) ON CONFLICT(name) DO UPDATE SET owner_pid = excluded.owner_pid, expires_at = excluded.expires_at").run(name, process.pid, expiresAt); })(); return async () => { db.prepare("DELETE FROM service_leases WHERE name = ? AND owner_pid = ?").run(name, process.pid); }; }
