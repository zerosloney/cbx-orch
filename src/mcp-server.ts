...
const tools = [
  { name: "cbx_start", description: "创建并后台执行一个任务", inputSchema: { type: "object", required: ["task"], properties: { task: { type: "string" }, context_snapshot: { type: "string", description: "父会话提炼的目标补充、计划、关键文件或命令输出及约束" }, task_contract: { type: "object", description: "可选结构化契约；提供后先执行上下文握手", properties: { goal: { type: "string" }, non_goals: { type: "array", items: { type: "string" } }, acceptance_criteria: { type: "array", items: { type: "string" } }, constraints: { type: "array", items: { type: "string" } }, relevant_files: { type: "array", items: { type: "string" } }, decisions: { type: "array", items: { type: "string" } }, rejected_options: { type: "array", items: { type: "string" } }, assumptions: { type: "array", items: { type: "string" } } } }, workspace: { type: "string" }, test_command: { type: "string" }, review: { type: "boolean" }, isolated: { type: "boolean" }, timeout_ms: { type: "number" }, max_retries: { type: "number" }, keep_worktree: { type: "boolean" }, priority: { type: "number" }, auto_branch: { type: "boolean" }, auto_commit: { type: "boolean" }, commit_message: { type: "string" }, executor: { type: "string", description: "内置执行器 codebuddy/opencode/omp，或插件路径" }, review_executor: { type: "string", description: "可选独立审查执行器；默认沿用 executor" } } } },
