...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === "cbx_start") {
    ...
    const job = await createJob({ workspace: root, task: String(args.task), contextSnapshot: args.context_snapshot === undefined ? undefined : String(args.context_snapshot), taskContract, testCommand: defaults.testCommand, review: defaults.review, isolated: defaults.isolated, permissionMode: defaults.permissionMode, maxTurns: defaults.maxTurns, timeoutMs: defaults.timeoutMs, maxRetries: defaults.maxRetries, keepWorktree: defaults.keepWorktree, reviewRules: config.reviewRules, approvalBeforeRun: defaults.approvalBeforeRun, autoBranch: defaults.autoBranch, autoCommit: defaults.autoCommit, commitMessage: defaults.commitMessage, executor: defaults.executor, reviewExecutor: defaults.reviewExecutor, allowUnsafePermissions: args.allow_unsafe_permissions === true });
