...
function adaptiveOverride(value: unknown): { enabled?: boolean; maxRounds?: number; managerExecutor?: string } | undefined {
  if (value === undefined) return undefined;
  if (!value || typeof value !== "object" || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) throw new Error("adaptive 必须是普通对象。");
  const raw = value as Record<string, unknown>;
  const unknown = Object.keys(raw).filter(key => !["enabled", "max_rounds", "manager_executor"].includes(key));
  if (unknown.length) throw new Error(`adaptive 不支持字段：${unknown.join(", ")}`);
  return { enabled: raw.enabled as boolean | undefined, maxRounds: raw.max_rounds as number | undefined, managerExecutor: raw.manager_executor as string | undefined };
}

const tools = [
  { name: "cbx_start", description: "创建并后台执行一个任务", inputSchema: { type: "object", required: ["task"], properties: { task: { type: "string" }, context_snapshot: { type: "string", description: "父会话提炼的目标补充、计划、关键文件或命令输出及约束" }, task_contract: { type: "object", description: "可选结构化契约；提供后先执行上下文握手", properties: { goal: { type: "string" }, non_goals: { type: "array", items: { type: "string" } }, acceptance_criteria: { type: "array", items: { type: "string" } }, constraints: { type: "array", items: { type: "string" } }, relevant_files: { type: "array", items: { type: "string" } }, decisions: { type: "array", items: { type: "string" } }, rejected_options: { type: "array", items: { type: "string" } }, assumptions: { type: "array", items: { type: "string" } }, stages: { type: "array", description: "接力链；每个 stage 用不同 executor，前序 stage 的 handback 自动注入下一个 stage", items: { type: "object", properties: { name: { type: "string" }, executor: { type: "string" }, task: { type: "string" }, review_executor: { type: "string" }, skip_review: { type: "boolean" } }, required: ["name", "executor", "task"] } } } }, workspace: { type: "string" }, test_command: { type: "string" }, review: { type: "boolean" }, isolated: { type: "boolean" }, timeout_ms: { type: "number" }, max_retries: { type: "number" }, keep_worktree: { type: "boolean" }, approval_before_complete: { type: "boolean" }, priority: { type: "number" }, auto_branch: { type: "boolean" }, auto_commit: { type: "boolean" }, commit_message: { type: "string" }, executor: { type: "string", description: "内置执行器 codebuddy/opencode/omp/cline，或插件路径" }, review_executor: { type: "string", description: "可选独立审查执行器；默认沿用 executor" }, adaptive: { type: "object", additionalProperties: false, properties: { enabled: { type: "boolean" }, max_rounds: { type: "integer", minimum: 1, maximum: 100 }, manager_executor: { type: "string" } } }, allow_unsafe_permissions: { type: "boolean", description: "permissionMode 为 dontAsk 时必须显式传 true（对应 CLI 的 --dangerously-skip-permissions）" } } } },
  ...
  { name: "cbx_continue", description: "根据审查意见继续任务", inputSchema: { type: "object", required: ["job_id"], additionalProperties: false, properties: { job_id: { type: "string" }, workspace: { type: "string" }, message: { type: "string" }, context_snapshot: { type: "string", description: "覆盖父会话提炼的目标补充、计划、关键文件或命令输出及约束" }, refresh_baseline: { type: "boolean", description: "确认当前 HEAD 为新的任务基线" }, extra_rounds: { type: "integer", minimum: 1, maximum: 100, description: "仅在 max_rounds Human Gate 等待时追加轮次" }, priority: { type: "number" } } } },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === "cbx_start") {
    ...
    if (args.approval_before_complete !== undefined && typeof args.approval_before_complete !== "boolean") throw new Error("approval_before_complete 必须是布尔值。");
    const defaults = mergeConfig(config, { testCommand: args.test_command ? String(args.test_command) : undefined, review: typeof args.review === "boolean" ? args.review : undefined, isolated: typeof args.isolated === "boolean" ? args.isolated : undefined, timeoutMs: args.timeout_ms === undefined ? undefined : Number(args.timeout_ms), maxRetries: args.max_retries === undefined ? undefined : Number(args.max_retries), keepWorktree: args.keep_worktree === undefined ? undefined : Boolean(args.keep_worktree), approvalBeforeComplete: typeof args.approval_before_complete === "boolean" ? args.approval_before_complete : undefined, autoBranch: args.auto_branch === undefined ? undefined : Boolean(args.auto_branch), autoCommit: args.auto_commit === undefined ? undefined : Boolean(args.auto_commit), commitMessage: args.commit_message ? String(args.commit_message) : undefined, executor: args.executor ? String(args.executor) : undefined, reviewExecutor: args.review_executor ? String(args.review_executor) : undefined, adaptive: adaptiveOverride(args.adaptive) });
    ...
    const taskContract: TaskContract | undefined = rawContract ? { goal: rawContract.goal as string | undefined, nonGoals: strings("non_goals"), acceptanceCriteria: strings("acceptance_criteria"), constraints: strings("constraints"), relevantFiles: strings("relevant_files"), decisions: strings("decisions"), rejectedOptions: strings("rejected_options"), assumptions: strings("assumptions"), stages: Array.isArray(rawContract.stages) ? rawContract.stages.map(stage => {
      if (!stage || typeof stage !== "object" || Array.isArray(stage)) return stage;
      const value = stage as Record<string, unknown>;
      return { name: value.name, executor: value.executor, task: value.task, reviewExecutor: value.review_executor, skipReview: value.skip_review };
    }) : rawContract.stages } as TaskContract : undefined;
    const job = await createJob({ workspace: root, task: String(args.task), contextSnapshot: args.context_snapshot === undefined ? undefined : String(args.context_snapshot), taskContract, testCommand: defaults.testCommand, review: defaults.review, isolated: defaults.isolated, permissionMode: defaults.permissionMode, maxTurns: defaults.maxTurns, timeoutMs: defaults.timeoutMs, maxRetries: defaults.maxRetries, keepWorktree: defaults.keepWorktree, reviewRules: config.reviewRules, approvalBeforeRun: defaults.approvalBeforeRun, approvalBeforeComplete: defaults.approvalBeforeComplete, autoBranch: defaults.autoBranch, autoCommit: defaults.autoCommit, commitMessage: defaults.commitMessage, executor: defaults.executor, reviewExecutor: defaults.reviewExecutor, adaptive: defaults.adaptive, allowUnsafePermissions: args.allow_unsafe_permissions === true });
  ...
  if (name === "cbx_continue") {
    if (args.extra_rounds !== undefined && (!Number.isInteger(args.extra_rounds) || Number(args.extra_rounds) < 1 || Number(args.extra_rounds) > 100)) throw new Error("extra_rounds 必须是 1 到 100 的整数。");
    await startBackground(root, id, String(args.message ?? "请根据 review.md 修复问题。"), Number(args.priority ?? 0), args.context_snapshot === undefined ? undefined : String(args.context_snapshot), Boolean(args.refresh_baseline), Number(args.extra_rounds ?? 0));
    return { job_id: id, status: "queued" };
  }
  if (name === "cbx_cancel") return cancelJob(root, id);
  if (name === "cbx_approve") { const state = await approveJob(root, id); if (state.status === "queued") await startBackground(root, id); return state; }
  if (name === "cbx_logs") {
