...
import { loadJson, now, processAlive, saveJson, withFileLock } from "./storage.js";
...
export interface QueueRuntime {
  ...
  loadState(workspace: string, jobId: string): Promise<{ status: string }>;
...
function queueFile(workspace: string): string { return path.join(workspace, ".cbx", "queue.json"); }
...
async function loadQueue(workspace: string): Promise<QueueFile> {
  const queue = await loadJson<QueueFile>(queueFile(workspace), { maxConcurrent: 2, paused: false, entries: [], updatedAt: now() });
...
async function saveQueue(workspace: string, queue: QueueFile): Promise<void> {
  queue.updatedAt = now();
  await saveJson(queueFile(workspace), queue);
}
...
export async function retryQueueJob(runtime: QueueRuntime, workspaceInput: string, jobId: string, priority = 0): Promise<QueueEntry> {
  ...
  await runtime.writeState(workspace, jobId, { status: "queued", phase: "queued", error: null, timedOut: false });
  return enqueueJob(runtime, workspace, jobId, "请读取已有的 test.log、review.md 和 result.json，修复失败原因后重新执行。", priority);
}
